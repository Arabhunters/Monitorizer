...
API_BASE = "/api/v1/"
API_SCAN = f"{API_BASE}scans"
API_TARGET = f"{API_BASE}targets"
...
class Acunetix(object):
    def __init__(self, host=None, api=None, timeout=20):
        self.apikey = api
        self.host = f'{"https://" if "https://" not in host else ""}{host}'
    ...
    def add_target(self, target="", criticality="normal"):
        if criticality not in target_criticality_allowed:
            raise AXException(
                ...
                f"Criticallity not found allowed values {list(target_criticality_allowed)}",
            )
        target_address = (
            target
    ...
    def start_scan(self, address=None, target_id=None, scan_profile="full_scan"):
        if scan_profile not in scan_profiles_allowed:
            raise AXException(
                ...
                f"Scan Profile not found allowed values {list(scan_profiles_allowed)}",
            )
...
class AcunetixReport():
    def acunetix(self, target):
        ...
        try:
            ...
            self.slack(f"acunetix integration had an error: {str(e)}")