...
class NucleiFuzzing(Report, Monitorizer, Console):
    ...
    def scan(self):
        ...
        cmd = f"./modules/nuclei/bin/nuclei -l {combined_waymore_output_for_nuclei_fuzzing} -no-color -silent -t ./modules/nuclei_fuzzing/fuzzing-templates/ -o {nuclei_fuzzing_output} {self.nuclei_fuzzing_options}"
        subprocess.check_output(cmd, shell=True)
